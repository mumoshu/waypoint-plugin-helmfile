...
func prepareBinaries(fs *PlatformConfig) (*string, *string, error) {
	conf := shoal.Config{
		Git: shoal.Git{
			Provider: "go-git",
		},
	...
	helmfileBin := fs.HelmfileBin

	helmfileVersion := fs.HelmfileVersion
	...
	if helmfileBin == "" {
		return nil, nil, errors.New("bug: helmfile.bin is missing")
	}
